...
usable_normal_2016_3sigma.iloc[:LEN,:].to_csv(f'usable_normal_2016_3sigma_{LEN}.csv', index=False)
usable_normal_2016_3sigma.iloc[LEN:,:].to_csv(f'usable_normal_2016_3sigma_after_{LEN}.csv', index=False)

usable_normal_2016_3sigma = pd.read_csv(f'usable_normal_2016_3sigma_{LEN}.csv')
...
usable_normal_2016_3sigma_after_3000 = pd.read_csv(f'usable_normal_2016_3sigma_after_{LEN}.csv')

zy = label_df[(label_df != 0).any(axis=1)] 
zx = attack_df[attack_df.index.isin(zy.index)]

zx = pd.concat([zx, usable_normal_2016_3sigma_after_3000])
zero_df = pd.DataFrame(0, index=usable_normal_2016_3sigma_after_3000.index, columns=usable_normal_2016_3sigma_after_3000.columns)
zy = pd.concat([zy, zero_df])

# 将攻击的结果和标签存储为两个CSV文件
# zx.to_csv(f'zx{id}.csv', index=False)
zy.to_csv(f'zy{id}{LEN}.csv', index=False)
...
df_normalized.to_csv(f'zx{id}{LEN}_normalized.csv', index=False)
