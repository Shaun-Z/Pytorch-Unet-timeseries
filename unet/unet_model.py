...
class UNet_1D_L(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        super(UNet_1D_L, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.down1 = (Down_1D(32, 64))
        self.down2 = (Down_1D(64, 128))
        self.up1 = (Up_1D(128, 64 // 1, bilinear))
        self.up2 = (Up_1D(64, 32 // 1, bilinear))
        self.outc = (OutConv_1D(32, n_classes))
...
class UNet_1D_N(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        ...
        factor = 2 if bilinear else 1
        self.down3 = (Down_1D(128, 256 // factor))
        self.down4 = (Down_1D(256, 512))

        self.up1 = (Up_1D(512, 256 // 1, bilinear))
        self.up2 = (Up_1D(256, 128 // 1, bilinear))
        self.up3 = (Up_1D(128, 64 // 1, bilinear))
        self.up4 = (Up_1D(64, 32 // 1, bilinear))
        self.outc = (OutConv_1D(32, n_classes))
...
class UNet_1D_NN(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        super(UNet_1D_NN, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.down3 = (Down_1D(128, 256 // factor))
        self.down4 = (Down_1D(256, 512))
        self.down5 = (Down_1D(512, 1024))


        self.up1 = (Up_1D(1024, 512 // 1, bilinear))
        self.up2 = (Up_1D(512, 256 // 1, bilinear))
        self.up3 = (Up_1D(256, 128 // 1, bilinear))
        self.up4 = (Up_1D(128, 64 // 1, bilinear))
        self.up5 = (Up_1D(64, 32 // 1, bilinear))
        self.outc = (OutConv_1D(32, n_classes))
...
class LSTMUnetGenerator(nn.Module):
    ...
    def __init__(self, batch_size, seq_len, feature_len, kernel_size, n_layers=1, hidden_dim=256):
        super().__init__()
        ...
        self.h_0 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_0 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.down1 = (Down_1D(32, 64))
        self.h_1 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_1 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.down2 = (Down_1D(64, 128))
        self.h_2 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_2 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.h_3 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_3 = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.h_1U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_1U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.h_2U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_2U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.h_3U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_3U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        ...
        self.h_4U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
        self.c_4U = torch.zeros(self.n_layers, batch_size, self.hidden_dim)
    ...
    def forward(self, x):
        ...
        # x2 = self.linear1(self.lstm1(self.down1(x1), (self.h_1, self.c_1)))
        # x3 = self.linear2(self.lstm2(self.down2(x2), (self.h_2, self.c_2)))
        # x4 = self.linear3(self.lstm3(self.down3(x3), (self.h_3, self.c_3)))
